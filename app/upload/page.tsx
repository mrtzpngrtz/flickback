  ...
  const upload = async () => {
    ...
    try {
      ...
      const createRes = await fetch('/api/videos', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ title: title.trim(), filename: file.name, storageKey, mimeType: file.type || 'video/mp4', size: file.size }),
      })
      ...
      setState({ status: 'error', progress: 0, error: err instanceof Error ? err.message : 'UPLOAD FAILED' })
    }
  }
      ...
      >
        ...
        {file ? (
          <div style={{ textAlign: 'center' }}>
            ...
            <div className="lbl" style={{ marginTop: 6 }}>{(file.size / 1024 / 1024).toFixed(1)} MB</div>
            {!isWorking && !isDone && (
              <button
        ...
        <input ref={inputRef} type="file" accept="video/*" style={{ display: 'none' }} onChange={e => { const f = e.target.files?.[0]; if (f) acceptFile(f) }} />
      </div>

      {file && !isWorking && !isDone && (
        <div style={{ marginBottom: 32 }}>
          <div className="lbl" style={{ marginBottom: 8 }}>TITLE</div>
          <input className="field" value={title} onChange={e => setTitle(e.target.value)} placeholder="VIDEO TITLE" style={{ fontWeight: 500 }} />
        </div>
      )}
      ...
      {isWorking && (
        <div style={{ marginBottom: 32 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
            <span className="lbl">{state.status === 'preparing' ? 'PREPARING' : state.status === 'uploading' ? 'UPLOADING' : 'SAVING'}</span>
            <span className="px accent">{String(Math.round(state.progress)).padStart(3, '0')}%</span>
          </div>
          <div className="progress-track">
            <div className={`progress-fill${state.status === 'saving' ? ' progress-fill--complete' : ''}`} style={{ width: `${state.progress}%` }} />
          </div>
        </div>
      )}

      {isDone && <div className="lbl" style={{ color: 'var(--accent)', marginBottom: 16 }}>DONE — OPENING REVIEW</div>}
      {state.status === 'error' && <div className="lbl" style={{ color: 'var(--accent)', marginBottom: 16 }}>ERROR: {state.error}</div>}

      {file && !isWorking && !isDone && (
        <button className="btn btn--primary" onClick={upload} disabled={!title.trim()} style={{ opacity: title.trim() ? 1 : 0.4 }}>
          UPLOAD
        </button>
      )}
    </main>
