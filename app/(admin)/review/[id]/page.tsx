...
export default async function ReviewPage({ params }: Props) {
  ...
  return (
    <>
      {/* Sub-header strips into the 40px topbar space */}
      <div
        style={{
          height: 36,
      ...
      >
        ...
        <span style={{ marginLeft: 'auto', display: 'flex', gap: 24, alignItems: 'center' }}>
          ...
          <span className={`status-dot status-dot--${video.status.toLowerCase()}`} />
        </span>
      </div>

      {/* Annotator fills remaining height — 40px topbar + 36px sub = 76px total */}
